...
#include <video/mipi_display.h> //csvke: other drivers use mipi_display.h

#include <drm/drm_crtc.h> // csvke: other drivers use drm_crtc.h
...
struct icna3512_panel_desc {
    ...
    unsigned int lanes;
    unsigned long mode_flags; // csvke: don't know why it wasn't defined in the original code
...
static int icna3512_enable(struct drm_panel *panel) 
{
    ...
    // Ensure the DSI controller is in LP11 mode
    dsi->mode_flags |= MIPI_DSI_MODE_LPM;

    // csvke: Initial commands to be sent to the panel
    ...
    dev_info(dev, "Exiting sleep mode\n");
    err = mipi_dsi_dcs_exit_sleep_mode(dsi);
    if (err < 0) {
        dev_err(dev, "Failed to exit sleep mode: %d\n", err);
        return err;
    }
    dev_info(dev, "Exited sleep mode successfully\n");

    // csvke: datasheet p.94, T5: Sleep-out command delay time min 80ms
    mdelay(80);

    dev_info(dev, "Setting display on\n");
    err = mipi_dsi_dcs_set_display_on(dsi);
    if (err < 0) {
        dev_err(dev, "Failed to set display on: %d\n", err);
        return err;
    }
    dev_info(dev, "Display set on successfully\n");

    // csvke: datasheet p.94, after T5: not sure if changing mode flag explicitly is necessary
...
static int icna3512_prepare(struct drm_panel *panel)
{
    ...
    // Set the DSI controller to LP11 mode
    dsi->mode_flags |= MIPI_DSI_MODE_LPM;

    // Ensure the data and clock lanes are in high state (LP11)
    // Add a delay to ensure the state is stable
    dev_info(panel->dev, "Setting DSI to LP11 mode and waiting for 2ms\n");
    usleep_range(2000, 2100); // 2ms to 2.1ms

    // Verify the state using debug messages
    dev_info(panel->dev, "DSI should now be in LP11 mode\n");
...
static int icna3512_dsi_probe(struct mipi_dsi_device *dsi)
{
    ...
    dev_info(dev, "Probing ICNA3512 DSI panel\n");

    icna3512 = devm_kzalloc(&dsi->dev, sizeof(*icna3512), GFP_KERNEL);
    if (!icna3512) {
        dev_err(dev, "Failed to allocate memory for icna3512\n");
        return -ENOMEM;
    }
    dev_info(dev, "Allocated memory for icna3512\n");
    ...
    dev_info(dev, "Retrieved device match data\n");

    // csvke: https://forums.raspberrypi.com/viewtopic.php?p=2276942&hilit=LP+11#p2276316
    icna3512->panel.prepare_prev_first = true;
    dev_info(dev, "Set panel prepare_prev_first to true\n");

    drm_panel_init(&icna3512->panel, dev, &icna3512_funcs, DRM_MODE_CONNECTOR_DSI);
    dev_info(dev, "Initialized DRM panel\n");

    // csvke: try to debug possible RP1 bug on DSI of not using burst mode https://github.com/raspberrypi/linux/pull/6292
    // dsi->mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_BURST | MIPI_DSI_MODE_NO_EOT_PACKET | MIPI_DSI_MODE_LPM;
    // csvke: rk3288_mipi_7D0_amoled.dts given by supplier has dsi,flags = <0xA03>;
    dsi->mode_flags = 0xA03;
    dsi->format = desc->format;
    dsi->lanes = desc->lanes;
    dev_info(dev, "Configured DSI mode flags, format, and lanes\n");
    ...
    dev_info(dev, "Got vddi regulator\n");
    ...
    dev_info(dev, "Got vci regulator\n");
    ...
    dev_info(dev, "Added DRM panel\n");

    mipi_dsi_set_drvdata(dsi, icna3512);
    icna3512->dsi = dsi;
    icna3512->desc = desc;
    dev_info(dev, "Set driver data\n");
    ...
    dev_info(dev, "Attached DSI device\n");
